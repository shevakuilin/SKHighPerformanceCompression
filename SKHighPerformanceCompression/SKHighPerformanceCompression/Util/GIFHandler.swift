...
class GIFHandler {
    ...
    static func loadGIF(with image: NSImage, onFinish: ((GIFRepresentation) -> ())) {
        ...
        guard let bitmapRep = image.representations[0] as? NSBitmapImageRep,
            ...
            let loopCount = (bitmapRep.value(forProperty: NSBitmapImageRep.PropertyKey.loopCount) as? NSNumber)?.intValue else {
        ...
        for n in 0 ..< frameCount {
            ...
            if let data = bitmapRep.representation(using: .gif, properties: [:]),
               let img = NSImage(data: data) {
                ...
                if let frameDuration = (bitmapRep.value(forProperty: NSBitmapImageRep.PropertyKey.currentFrameDuration) as? NSNumber)?.doubleValue {
    ...
    static func isAnimatedGIF(_ image: NSImage) -> Bool {
        ...
        guard let bitmapRep = image.representations[0] as? NSBitmapImageRep,
            ...
            let _ = (bitmapRep.value(forProperty: .currentFrame) as? NSNumber)?.floatValue else {
            return false
        }
